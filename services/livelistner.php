...
foreach($array_resq911 as $row_resq911){
	...
	if(mysqli_num_rows($sql_select) > 0){
		if($Number == $dbname){
			$update_devices = "UPDATE devices SET time = '$timefinal', lat = '$lat' , lng = '$lng', angle = '$VectorAngle' , ignition = '$ignition', speed ='$VectorSpeed' , odometer = '' , lasttime = '$timefinal', latestPosition_id = '$MessageId'  where id ='$ObjectId'";
		...
		echo "<br>";
			...
			$update_devices = "UPDATE devices SET name = '$Number', time = '$timefinal', lat = '$lat' , lng = '$lng', angle = '$VectorAngle' , ignition = '$ignition', speed ='$VectorSpeed' , odometer = '' , lasttime = '$timefinal', latestPosition_id='$MessageId'  where id ='$ObjectId'";
			$exe_Selectnew = mysqli_query( $connection,$update_devices);
			echo mysqli_error($connection);
			if($exe_Selectnew == True){
	...
	(id,time, lat, lng, angle, ignition, speed, odometer, lasttime, latestPosition_id, name)  
	VALUES 
	('$ObjectId','$timefinal','$lat','$lng','$VectorAngle','$ignition','$VectorSpeed','','$timefinal','$MessageId','$Number')") or die(mysqli_error($connection));
