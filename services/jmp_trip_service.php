...
$sql_main = "SELECT * FROM bsl.jmp_trips where complete_status=0";
// echo $sql_1 ."<br>";
...
if ($count_main > 0) {
    while ($row_main = mysqli_fetch_array($result_main)) {
        ...
        $sql_1 = "SELECT tc.id as trip_sub_id,tc.*,dc.lat,dc.lng,geo.Coordinates as origin_co,geo.type as origin_type,geo_d.Coordinates as des_co,geo_d.type as des_type FROM bsl.jmp_trips as tc 
        join devices as dc on dc.id=tc.vehicle_id 
        join geofenceing as geo on geo.id=tc.loading_site 
        join geofenceing as geo_d on geo_d.id=tc.destination 
        where tc.id='$trip_p_id' ";
        // echo $sql_1 ."<br>";
        ...
        if ($count_1 > 0) {
            while ($row = mysqli_fetch_array($result_1)) {
                $origin = $row['loading_site'];
                $vehicle = $row['vehicle_id'];
                $destination = $row['destination'];
                $status = $row['status'];
                $eta_hour = $row['eta_destination'];
                $trip_sub_id = $row['trip_sub_id'];
                $v_lat = $row['lat'];
                $v_lng = $row['lng'];
                $radius = 3;
                $Coordinates = $row['origin_co'];
                $origin_type = $row['origin_type'];
                $des_co = $row['des_co'];
                $des_type = $row['des_type'];
                $trip_start_time = $row['trip_start_date'];
                ...
                if ($status == 0) {
                   ...
                   echo $formatted_date_time . '<br>';
                    ...
                    //             echo $out_time . '<br>';
                    //             echo $eta_time . '<br>';
                    //             $update_departure = "UPDATE `bsl`.`jmp_trips`
                    //                 SET
                    //                 `status` = '1',
                        ...
                        if ($response != null) {
                            ...
                            foreach ($json_arr as $row_resq911) {
                                ...
                                if ($des_co != "") {
                                    ...
                                    if (arrival_checkin($lat, $lng, $des_co, $radius) == true) {
                                        // $out_time = date('Y-m-d H:i:s');
                                        $time = $row_resq911["time"];
                    ...
                    //     $update_arival = "UPDATE `bsl`.`jmp_trips`
                    //         SET
                    //         `status` = '2',
                    ...
                    if ($err) {
                        ...
                        if ($response != null) {
                            ...
                            foreach ($json_arr as $row_resq911) {
                                ...
                                if ($des_co != '') {
                                    if (delivery_checkin($lat, $lng, $des_co, $radius) == true) {
                                        // $out_time = date('Y-m-d H:i:s');
                                        $time = $row_resq911["time"];
         ...
         $sql_2 = "SELECT tc.id as trip_sub_id,tc.*,tc.*,dc.lat,dc.lng,geo.Coordinates,geo.type FROM bsl.jmp_trips as tc 
            join devices as dc on dc.id=tc.vehicle 
            join geofenceing as geo on tc.origin
            where tc.id='$trip_p_id' and tc.trip_status=0 order by tc.craated_at asc limit 1";
            // echo $sql_1 ."<br>";
