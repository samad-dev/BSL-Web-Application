...
<head>
    ...
    $result = mysqli_query($db, "SELECT * FROM `positions` where device_id = 1361 and time >='2021-10-04' ORDER BY `id` asc;");
    ...
    while ($user = $result->fetch_assoc()) {
        ...
        if ($first_time == "") {
            if ($user["speed"] == '0') {
                ...
                // echo 'Trip Started at '.$user["vlocation"].'-'.$user["time"].'</br>';
                $first_location = $user["vlocation"];
            ...
            if ($user["speed"] == '0') {
                ...
                if ($diff_minutes >= 30) {
                    ...
                    // echo 'Trip End  at '.$user["vlocation"].'-'.$user["time"].'</br>';
                    $inside['end_time'] = $user["time"];
                    $inside['end_location'] = $user["vlocation"];
                    $end_time = $user["time"];
                    $from_time = strtotime($start_time);
