...
$sql = "SELECT DATE_FORMAT(tt.start_time, '%Y-%m-%d %H:%i:%s') AS trip_start_time,tt.*,tc.*,tt.vehicle as vehi_id,geo.Coordinates as co_origin,geo_d.Coordinates as co_destination,geo.consignee_name as origin_name,geo_d.consignee_name as destination_name,dc.name as vehicle_name FROM bsl.trips as tt 
join trips_child as tc on tc.main_id=tt.id 
join geofenceing as geo on geo.id=tc.origin
join geofenceing as geo_d on geo_d.id=tc.destination
join devices as dc on dc.id=tt.vehicle
where tt.id='$trip_id' and tc.direction='UP'";
...
$trip_start_time = $row_allow['trip_start_time'];

if ($row_allow['trip_status'] == 1) {
    $delivery_time = $row_allow['complete_time'];
} else {
...
$(document).ready(function() {
    myFunction("<?php echo $row_allow['vehicle_name'] ?>", "<?php echo $row_allow['vehi_id'] ?>",
        "<?php echo $trip_start_time ?>", "<?php echo $delivery_time ?>");
...
function initMap() {
    ...
    var destination = "<?php echo $row_allow['co_destination'];?>";
    var des_Array = destination.split(", ");

    var origin = "<?php echo $row_allow['co_origin'];?>";
    var origin_Array = origin.split(", ");
    marker_creation(des_Array[0], des_Array[1], "<?php echo $row_allow['destination_name'] ?>","1","Destination");
    marker_creation(origin_Array[0], origin_Array[1], "<?php echo $row_allow['origin_name'] ?>","1","Origin");
}
